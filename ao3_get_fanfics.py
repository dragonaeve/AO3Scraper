...
def get_single_comment(comment, parent):
	...
	commentData = {'user': username, 'datetime': dateObj, 'id': commentid, 'parent': parent, 'text': unidecode(fullText)}
	return commentData
		...
		with open("errors_" + csv_out, 'a') as e_out:
			...
			if os.stat(csv_out).st_size == 0:
				...
				header = ['work_id', 'title', 'author', 'rating', 'category', 'fandom', 'relationship', 'character',
						  ...
						  'comments', 'kudos', 'bookmarks', 'hits', 'all_kudos', 'all_bookmarks', 'all_comments',
						  'body']
